/*
 * File: toddler.js
 * Created on Sat Sep 21 21:22:21 PDT 2019 by java2js
 * --------------------------------------------------
...
var TDCellView = function() {
   if (!(this instanceof TDCellView)) return new TDCellView();
   /* Empty */
};
...
var TDCommandListener = function(tdi, cl) {
   if (!(this instanceof TDCommandListener)) return new TDCommandListener(tdi, cl);
   this.tdi = tdi;
   this.cl = cl;
};
...
var TDCommandLoop = function(td, console) {
   if (!(this instanceof TDCommandLoop)) return new TDCommandLoop(td, console);
...
var TDLoadCommandListener = function(tdi, filename) {
   if (!(this instanceof TDLoadCommandListener)) return new TDLoadCommandListener(tdi, filename);
   this.tdi = tdi;
...
var TDViewCanvas = function() {
   if (!(this instanceof TDViewCanvas)) return new TDViewCanvas();
   JSCanvas.call(this);
...
var TDCPUView = function(model) {
   if (!(this instanceof TDCPUView)) return new TDCPUView(model);
...
var TDACView = function(model, x, y) {
   if (!(this instanceof TDACView)) return new TDACView(model, x, y);
...
var PCView = function(model, x, y) {
   if (!(this instanceof PCView)) return new PCView(model, x, y);
   TDCellView.call(this);
   this.model = model;
...
var TDIRView = function(model, x, y) {
   if (!(this instanceof TDIRView)) return new TDIRView(model, x, y);
   TDCellView.call(this);
   this.model = model;
...
var TDInsView = function(model, x, y) {
   if (!(this instanceof TDInsView)) return new TDInsView(model, x, y);
   TDCellView.call(this);
   this.model = model;
...
var TDXRView = function(model, x, y) {
   if (!(this instanceof TDXRView)) return new TDXRView(model, x, y);
   TDCellView.call(this);
   this.model = model;
   var width = model.getAddressSize() * TDC.DIGIT_WIDTH + TDC.SHORT_PADDING;
   this.setBounds(x, y, width, TDC.CELL_HEIGHT);
...
var TDSPView = function(model, x, y) {
   if (!(this instanceof TDSPView)) return new TDSPView(model, x, y);
   TDCellView.call(this);
   this.model = model;
   var width = model.getAddressSize() * TDC.DIGIT_WIDTH + TDC.SHORT_PADDING;
   this.setBounds(x, y, width, TDC.CELL_HEIGHT);
...
var TDEditor = function(model) {
   if (!(this instanceof TDEditor)) return new TDEditor(model);
...
var TDInterpreter = function(console) {
   if (!(this instanceof TDInterpreter)) return new TDInterpreter(console);
...
var TDLayout = function() {
   if (!(this instanceof TDLayout)) return new TDLayout();
   /* Empty */
};
...
var TDMemoryView = function(model) {
   if (!(this instanceof TDMemoryView)) return new TDMemoryView(model);
   JSPanel.call(this);
   this.model = model;
...
var TDMemoryCanvas = function(model) {
   if (!(this instanceof TDMemoryCanvas)) return new TDMemoryCanvas(model);
   TDViewCanvas.call(this);
   this.model = model;
...
var TDMemoryCellView = function(model, offset, x, y) {
   if (!(this instanceof TDMemoryCellView)) return new TDMemoryCellView(model, offset, x, y);
   TDCellView.call(this);
   this.model = model;
   this.offset = offset;
   var width = model.getWordSize() * TDC.DIGIT_WIDTH;
...
var TDColumnHeader = function(memoryView) {
   if (!(this instanceof TDColumnHeader)) return new TDColumnHeader(memoryView);
   JSCanvas.call(this);
...
var TDRowHeader = function(memoryView) {
   if (!(this instanceof TDRowHeader)) return new TDRowHeader(memoryView);
   JSCanvas.call(this);
   /* Empty */
};
...
var TDModel = function() {
   if (!(this instanceof TDModel)) return new TDModel();
...
TDModel.prototype.step = function() {
   ...
   if (this.pc === 0) {
      ...
      try {
         ...
         var html = "<html>" + RuntimeException.patchMessage(ex) + "</html>";
         this.editor.showErrorDialog(html, pcLine);
         this.setPC(0);
...
var TDEditorChangeListener = function(model) {
   if (!(this instanceof TDEditorChangeListener)) return new TDEditorChangeListener(model);
   this.model = model;
};
...
var Toddler = function() {
   if (!(this instanceof Toddler)) return new Toddler();
   JSProgram.call(this);
...
//   this.chooser.addLoadListener(new TDLoadListener(this, this.chooser));
//   this.chooser.addSaveListener(new TDSaveListener(this, this.chooser));
...
Toddler.prototype.updateChooser = function() {
   if (JSProgram.isJavaScript()) {
      var path = "cgi:" + this.getUID() + "/Toddler";
...
Toddler.prototype.actionPerformed = function(e) {
   ...
   if (jslib.equals(cmd, "Load")) {
      ...
      dialog.type = "file";
      dialog.addEventListener("change", dialogAction, false);
      dialog.click();
   ...
   function dialogAction() {
      ...
      if (files.length > 0) {
         ...
         rd.onload = function() {
            var editor = td.editor;
            editor.removeAllBreakpoints();
            editor.setText(rd.result);
            editor.setCursorPosition(0);
            var frame = editor.getFrame();
            if (frame !== null) frame.setTitle(JSFile.getTail(files[0].name));
         };
...
var TDLoadListener = function(pgm, chooser) {
   if (!(this instanceof TDLoadListener)) return new TDLoadListener(pgm, chooser);
   this.pgm = pgm;
   this.chooser = chooser;
};

TDLoadListener.prototype.actionPerformed = function(e) {
   if (e instanceof JSErrorEvent) {
      // Fill in
   } else {
      var path = this.chooser.getPath();
...
var TDLoadFileListener = function(pgm, path) {
   if (!(this instanceof TDLoadFileListener)) return new TDLoadFileListener(pgm, path);
   this.pgm = pgm;
   this.path = path;
};
...
var TDSaveListener = function(pgm, chooser) {
   if (!(this instanceof TDSaveListener)) return new TDSaveListener(pgm, chooser);
   this.pgm = pgm;
   this.chooser = chooser;
};

TDSaveListener.prototype.actionPerformed = function(e) {
   if (e instanceof JSErrorEvent) {
      // Fill in
   } else {
      var path = this.chooser.getPath();
...
var TDSaveFileListener = function(pgm, path) {
   if (!(this instanceof TDSaveFileListener)) return new TDSaveFileListener(pgm, path);
   this.pgm = pgm;
   this.path = path;
};
